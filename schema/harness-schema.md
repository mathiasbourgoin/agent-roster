...
The `harness.json` file is the project-level manifest describing the complete Claude Code harness configuration. Generated by the **tech-lead** agent during install, maintained by layer-specific agents.
...
Each layer is populated and maintained by a specific agent.
...
Stored at `.claude/harness.json` in the project root. Agents read this to understand the current harness state before making changes.
